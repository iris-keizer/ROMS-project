...
def import_data():
    ...
    # Import grid
    grid = xr.open_dataset('/Users/iriskeizer/Documents/ROMS/code/model2roms4/NorthSea4_smooth01_sponge_nudg.nc')
    
    # Import data
    data = xr.open_dataset('/Users/iriskeizer/Documents/ROMS/data/ORA20C/forcing input/ora20c.nc')
    
    data = data.where(data.time.dt.year > 1949, drop = True)
...
def horizontal_interp(input_data, grid):
    ...
    output_data.to_netcdf('/Users/iriskeizer/Documents/ROMS/data/lateral forcing/NorthSea4 ORA20C/data_hr.nc')
...
def vertical_transf(data, grid):
    ...
    result.to_netcdf('/Users/iriskeizer/Documents/ROMS/data/lateral forcing/NorthSea4 ORA20C/data_hr_vt.nc')
...
def vert_integrate_mom(data, grid):
    ...
    result.to_netcdf('/Users/iriskeizer/Documents/ROMS/data/lateral forcing/NorthSea4 ORA20C/result.nc')
...
ocean_time = river.river_time[24:-120].data
...
# Change NaN values to ROMS grid value 
result['u'] = result.u.where(grdROMS.mask_u == 1, grdROMS.fillval)
result['v'] = result.v.where(grdROMS.mask_v == 1, grdROMS.fillval)

result['salt'] = result.salt.where(grdROMS.mask_rho == 1, grdROMS.fillval)
result['temp'] = result.temp.where(grdROMS.mask_rho == 1, grdROMS.fillval)
result['zeta'] = result.zeta.where(grdROMS.mask_rho == 1, grdROMS.fillval)

result['ubar'] = result.ubar.where(grdROMS.mask_u == 1, grdROMS.fillval)
result['vbar'] = result.vbar.where(grdROMS.mask_v == 1, grdROMS.fillval)
...
start_date = f'{data.time[0].dt.year.data}{data.time[0].dt.month.data}{data.time[0].dt.day.data}'
final_date = f'{data.time[-1].dt.year.data}{data.time[-1].dt.month.data}{data.time[-1].dt.day.data}'


init.to_netcdf(f'{path}/NorthSea4_init_{data_type}_{start_date}_to_{final_date}.nc')
bry.to_netcdf(f'{path}/NorthSea4_bry_{data_type}_{start_date}_to_{final_date}.nc')
clim.to_netcdf(f'{path}/NorthSea4_clim_{data_type}_{start_date}_to_{final_date}.nc')
