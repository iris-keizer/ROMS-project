...
def getTime(confM2R, year, month, day, ntime):
    ...
    if confM2R.indatatype == 'NORESM':
        ...
        jdref = date2num(datetime(1948, 1, 1), cdf.variables["time"].units,
                         calendar=cdf.variables["time"].calendar) #updated for glorys12v1 Tim Hermans 15-10-19
    ...
    if confM2R.indatatype == 'GLORYS':
        ...
        mycalendar = cdf.variables["time"].calendar #updated for glorys12v1 Tim Hermans 15-10-19
        myunits = cdf.variables["time"].units
        #myunits = "hours since 1950-01-01 00:00:00"
        currentdate = datetime(year, month, day)
        jd = date2num(currentdate, myunits, calendar=mycalendar)
        ...
        for month in months:
            ...
            for day in days:
                ...
                for myvar in confM2R.globalvarnames:
                    ...
                    if myvar in ['temperature','salinity','uvel','vvel','O3_c','O3_TA','N1_p','N3_n','N5_s','O2_o']: #try something: replace myvar with uvel, ty to interpolate uvel on simple rho grid- see what happens
